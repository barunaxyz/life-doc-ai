...
from mcp.notion_client import get_life_events, get_journal_entries, get_goals
from agent.prompts import DOCUMENTARY_PROMPT, TIMELINE_PROMPT, INSIGHT_PROMPT, CHAPTERS_PROMPT
...
class DocumentaryAgent:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Here is the data: {json.dumps(user_data)}"},
                ],
    ...
    def _fallback_chapters(self, events):
        ...
        chapter_names = ["The Beginning", "Building Foundations", "Rising Momentum",
                         "The Breakthrough", "New Horizons", "The Next Chapter"]
        html = ""
    ...
    def _fallback_insights(self, events, journals):
        ...
        if events:
            ...
            if years:
                ...
                items.append(("📅", "Most Active Year", f"{most_active[0]} with {most_active[1]} events", "#7C3AED"))

            high_impact = [e for e in events if e.get("impact") == "High"]
            pct = round(len(high_impact) / len(events) * 100)
            items.append(("🔴", "High-Impact Events", f"{len(high_impact)} out of {len(events)} ({pct}%)", "#EF4444"))

            first_year = min(years) if years else "?"
            last_year = max(years) if years else "?"
            span = int(last_year) - int(first_year) + 1 if years else 0
            items.append(("📊", "Journey Span", f"{span} years ({first_year} — {last_year})", "#06B6D4"))

        if journals:
            items.append(("📓", "Journal Entries", f"{len(journals)} reflections documented", "#EC4899"))
            avg_len = sum(len(j.get("content", "")) for j in journals) // max(len(journals), 1)
            items.append(("✍️", "Avg. Entry Length", f"~{avg_len} characters per entry", "#F59E0B"))
    ...
    def generate_documentary(self):
        ...
        print(f"  → Found {len(events)} events, {len(journals)} journals, {len(goals)} goals")
